...
export async function generateVMPPages(pool: Pool, dist: string) {
  ...
  const { rows } = await pool.query(`
    SELECT v.code, v.name, v.abbreviated_name, v.vtm_code, v.vmp_group_code, v.status,
      ...
      (SELECT json_build_object('code', vtm.code, 'name', vtm.name) FROM vtm WHERE vtm.code = v.vtm_code) as vtm,
      (SELECT json_build_object('code', vg.code, 'name', vg.name, 'patientFrailtyIndicator', vg.patient_frailty_indicator)
       FROM vmp_group vg WHERE vg.code = v.vmp_group_code) as vmp_group,
      (SELECT COALESCE(json_agg(json_build_object(
        'code', amp.code, 'name', amp.name, 'companyName', c.denomination, 'blackTriangle', amp.black_triangle
      ) ORDER BY amp.name->>'en'), '[]'::json)
      FROM amp LEFT JOIN company c ON c.actor_nr = amp.company_actor_nr
      WHERE amp.vmp_code = v.code AND (amp.end_date IS NULL OR amp.end_date > CURRENT_DATE)) as amps,
      (SELECT COUNT(DISTINCT ampp.cti_extended)::int FROM ampp JOIN amp ON amp.code = ampp.amp_code
       WHERE amp.vmp_code = v.code AND (ampp.end_date IS NULL OR ampp.end_date > CURRENT_DATE)) as package_count,
      ...
      (SELECT CASE WHEN COUNT(DISTINCT d.code) = 0 THEN NULL
       ...
       AND (d.end_date IS NULL OR d.end_date > CURRENT_DATE) AND (ampp.end_date IS NULL OR ampp.end_date > CURRENT_DATE)) as reimbursable_percentage
    FROM vmp v WHERE v.end_date IS NULL OR v.end_date > CURRENT_DATE ORDER BY v.name->>'en'`);
